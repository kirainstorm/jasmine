...
//   coreManager->unlockVideoRender();
  ...
  linphone_core_set_native_preview_window_id(linphonecx::get_core(), nullptr);
...
void CameraPreviewRenderer::render () {
  // Draw with ms filter.
  {
   ...
   // CoreManager *coreManager = CoreManager::getInstance();
	...
	linphone_core_preview_ogl_render(linphonecx::get_core());
...
void CameraPreviewRenderer::updateWindowId () {
  ...
  linphone_core_set_native_preview_window_id(linphonecx::get_core(),mContextInfo);
}
...
CameraPreview::CameraPreview (QQuickItem *parent) : QQuickFramebufferObject(parent) {
  mCounterMutex.lock();
  //linphone_core_enable_video(linphonecx::get_core(), true, true);
  if (++mCounter == 1)
	  linphone_core_enable_video_preview(linphonecx::get_core(), true);
...
CameraPreview::~CameraPreview () {
  mCounterMutex.lock();
  if (--mCounter == 0)
	  linphone_core_enable_video_preview(linphonecx::get_core(), false);
